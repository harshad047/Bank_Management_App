					...
					<form method="get" action="dashboard" class="d-flex">
						<select name="filter" class="form-select form-select-sm me-2"
							...
							<option value="APPROVED"
								${selectedFilter == 'APPROVED' ? 'selected' : ''}>Approved</option>
							<option value="REJECTED"
								${selectedFilter == 'REJECTED' ? 'selected' : ''}>Rejected</option>
							<option value="DEACTIVATED"
								${selectedFilter == 'DEACTIVATED' ? 'selected' : ''}>Deactivated</option>
							<option value="CLOSED"
								${selectedFilter == 'CLOSED' ? 'selected' : ''}>Closed</option>
							<option value="IN_PROGRESS"
								${selectedFilter == 'IN_PROGRESS' ? 'selected' : ''}>In
								Progress</option>
							<option value="PENDING"
								${selectedFilter == 'PENDING' ? 'selected' : ''}>Pending</option>
							<option value="RESOLVED"
								${selectedFilter == 'RESOLVED' ? 'selected' : ''}>Resolved</option>
						</select>
					</form>
				</div>
				<div class="card-body">
